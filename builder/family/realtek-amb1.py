...
env.Replace(
    SDK_SYSTEM_DIR=join(env.subst("$SDK_DIR"), "Arduino_package", "hardware", "system"),
    SDK_VARIANT_DIR=join(env.subst("$SDK_DIR"), "Arduino_package", "hardware", "variants", "rtl8710"),
)
...
queue.AppendPublic(
    ...
    CPPDEFINES=[
        ...
        ("FLASH_OTA1_OFFSET", "0"),
        ("FLASH_OTA2_OFFSET", "0"),
        ("FLASH_SYSTEM_OFFSET", "0"),
        ("INCLUDE_xTimerGetTimerDaemonTaskHandle", "1"),
        ("ERRNO", "1"),
...
queue.AppendPublic(
    CPPPATH=[
        ...
        join("$SDK_SYSTEM_DIR", "component", "common", "network", "lwip", "lwip_v1.4.1", "src", "include", "ipv4"),
...
env.Replace(
    LTCHIPTOOL='cd /tmp && ${LTPYTHONEXE} -m ltchiptool -r -i 1 -L "${LT_DIR}"',
    LINK="arm-none-eabi-gcc",
    UF2OTA=[f"{image_app}=flasher:app"],
)
